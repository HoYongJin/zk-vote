...
GITHUB_WORKFLOW_REF="${GITHUB_WORKFLOW_REF:-${GITHUB_REPOSITORY}/.github/workflows/deploy-production.yml@${GITHUB_REF}}"
POOL_ID="${WIF_POOL_ID:-zkvote-github}"
PROVIDER_ID="${WIF_PROVIDER_ID:-github}"
SERVICE_ACCOUNT="${GCP_CI_DEPLOY_SERVICE_ACCOUNT:-zkvote-prod-ci-deployer@${PROJECT_ID}.iam.gserviceaccount.com}"
LOCATION="global"
...
if ! gcloud iam workload-identity-pools describe "${POOL_ID}" \
  ...
  gcloud iam workload-identity-pools create "${POOL_ID}" \
    ...
    --description "GitHub Actions OIDC identity for zk-vote production deploys" \
    --quiet
fi
