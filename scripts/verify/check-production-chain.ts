#!/usr/bin/env tsx
/**
 * Read-only production chain/key gate.
 *
 * Reads production secrets in-process, derives public addresses without placing
...
const DEFAULT_SECRET_NAMES = {
    ...
    relayerPrivateKey: "zkvote-prod-relayer-private-key",
...
async function main(): Promise<void> {
    ...
    const evidencePath =
        ...
        path.join(PROJECT_ROOT, "docs", "evidence", `production-chain-${runId}.json`);
    const checkLabel = env("CHAIN_CHECK_LABEL", "production chain check");
