...
const DEFAULT_SECRET_NAMES = {
    firebaseApiKey: "zkvote-prod-firebase-web-api-key",
...
async function maybeVerifyContractsOnEtherscan(
    ...
    const script = optionalEnv("ETHERSCAN_VERIFY_SCRIPT") ?? "scripts/verify/verify-production-contracts-etherscan.ts";
    const evidencePath =
...
async function main(): Promise<void> {
    ...
    const configuredBaseUrl = optionalEnv("VERIFY_BASE_URL") ?? optionalEnv("PRODUCTION_BASE_URL") ?? optionalEnv("PROD_BASE_URL");
    const baseUrl = (configuredBaseUrl ?? (await execFile(
        "gcloud",
        ["run", "services", "describe", service, "--project", projectId, "--region", region, "--format=value(status.url)"],
        { cwd: PROJECT_ROOT }
    )).stdout.trim()).replace(/\/$/, "");
    assert(baseUrl, "Cloud Run service URL is empty");
    const artifactBucket = env("ARTIFACT_BUCKET", `zkvote-prod-artifacts-${projectId}`);
    ...
    try {
        ...
        const electionName = `production-e2e-${runId}`;
        ...
        const registration = await apiRequest<Json>(
            ...
            {
                token: voter.idToken,
                body: { name: "Production E2E Voter", secretCommitment },
            }
        );
        ...
        assert(
            ...
            "fresh production proof did not verify locally"
        );
        ...
        console.log(`production E2E PASSED; evidence=${evidencePath}`);
    } catch (error) {
