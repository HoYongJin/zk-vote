#!/usr/bin/env tsx
/**
 * Read-only production load smoke.
 *
...
async function resolveBaseUrl(projectId: string): Promise<string> {
    const configured = optionalEnv("VERIFY_BASE_URL") ?? optionalEnv("PRODUCTION_BASE_URL") ?? optionalEnv("PROD_BASE_URL");
    ...
    const { stdout } = await execFile(
        ...
        [
            ...
            "services",
            "describe",
            env("CLOUD_RUN_SERVICE", DEFAULT_SERVICE),
            "--project",
            projectId,
            "--region",
            env("GCP_REGION", DEFAULT_REGION),
            "--format=value(status.url)",
        ],
        { maxBuffer: 1024 * 1024 }
    );
    const url = stdout.trim().replace(/\/$/, "");
    assert(url, "Cloud Run service URL is empty");
    return url;
}
...
async function main(): Promise<void> {
    ...
    const baseUrl = await resolveBaseUrl(projectId);
    ...
    const evidencePath =
        ...
        path.join(PROJECT_ROOT, "docs", "evidence", `production-load-readonly-${runId}.json`);
    const checkLabel = env("LOAD_CHECK_LABEL", "production read-only load");
