...
function AuthHandler({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const fetchRole = async (): Promise<{
      ...
      try {
        const data = await queryClient.fetchQuery<MeResponse>({
        ...
        return {
          ...
          appUserId: data.id,
          backendEmail: data.email,
        };
      } catch (error) {
        console.error('Failed to resolve role from /api/me:', errorData(error));
        return { isAdmin: false, isSuperAdmin: false, appUserId: null, backendEmail: null }; // fail-closed
      }
    };

    // 관리자인지 확인하고, 로그인 직후라면 적절한 페이지로 리디렉션한다.
    const checkAdminAndRedirect = async (): Promise<void> => {
      const { isAdmin, isSuperAdmin, appUserId, backendEmail } = await fetchRole();
      dispatch(setAdmin({ isAdmin, isSuperAdmin, appUserId, backendEmail }));
