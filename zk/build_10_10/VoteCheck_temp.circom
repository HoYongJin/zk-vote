...
template MerkleProof(depth) {
    ...
    signal term1_left[depth];
    signal term2_left[depth];
    signal term1_right[depth];
    signal term2_right[depth];
    ...
    for (var i = 0; i < depth; i++) {
        ...
        term1_right[i] <== pathIndices[i] * cur[i];
        term2_right[i] <== (1 - pathIndices[i]) * pathElements[i];
        right[i] <== term1_right[i] + term2_right[i];
...
component main {public [election_id]} = Main(10, 10);